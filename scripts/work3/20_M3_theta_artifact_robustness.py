...
OUT  = os.path.join(ROOT, "outputs", "work3", "20_M3_theta")
...
def clean_session(win, do_asr):
    """Return (cleaned_windows, n_ic_removed, ic_types). do_asr adds ASR after ICA."""
    n, C, T = win.shape
    raw = make_raw(win)
    raw_hp = raw.copy().filter(1., None, verbose="ERROR")   # ICA prefers ~1 Hz high-pass
    ica = mne.preprocessing.ICA(n_components=0.99, method="infomax",
                                fit_params=dict(extended=True),
                                max_iter="auto", random_state=0, verbose="ERROR")
    ica.fit(raw_hp)
    exclude, ictypes = set(), []
    # ocular via frontal proxy channels
    for ch in FRONTAL:
        try:
    ...
    # muscle
    try:
        idx, _ = ica.find_bads_muscle(raw_hp, verbose="ERROR")
        for i in idx: exclude.add(i); ictypes.append(("muscle", "", i))
    except Exception: pass
    # ICLabel refinement (if installed): drop eye/muscle/line/channel-noise > 0.70
    if HAVE_ICLABEL:
        try:
            lab = label_components(raw_hp, ica, method="iclabel")
            for i, (lb, pr) in enumerate(zip(lab["labels"], lab["y_pred_proba"])):
                if lb in ("eye blink", "muscle artifact", "line noise", "channel noise") and pr > 0.70:
                    exclude.add(i); ictypes.append((lb, "iclabel", i))
        except Exception: pass
    ica.exclude = sorted(exclude)
    raw_c = raw.copy(); ica.apply(raw_c, verbose="ERROR")   # reconstruct on UNfiltered raw
    cleaned = raw_c.get_data()
    # optional ASR on top
    if do_asr and ASR is not None:
        try:
            if ASR[0] == "asrpy":
                asr = ASR[1](sfreq=FS, cutoff=20)
                asr.fit(raw_c.copy().filter(1., None, verbose="ERROR"))
                cleaned = asr.transform(raw_c).get_data()
            else:  # meegkit
                asr = ASR[1](method="euclid", sfreq=FS)
                asr.fit(cleaned)
                cleaned, _ = asr.transform(cleaned)
        except Exception as e:
            print(f"   [asr] skipped ({e})")
    out = cleaned.reshape(len(CH), n, T).transpose(1, 0, 2).astype(np.float32)
    return out, len(ica.exclude), ictypes
...
STAGES = ["stored", "filter", "ica", "ica_asr"]
theta_tbl = {st: {} for st in STAGES}     # (session, emotion) -> region-theta dict
ic_removed = {}
for sid in sorted(sess):
    Xs, es = subsample(sid)
    print(f"\n[session {sid}] cleaning {len(Xs)} sampled windows ...")
    # stored
    for e in EMOS:
        m = es == e
        if m.sum() >= 5: theta_tbl["stored"][(sid, e)] = region_theta(Xs[m])
    # filter-only
    raw_hp = make_raw(Xs).filter(1., None, verbose="ERROR").get_data()
    Xf = raw_hp.reshape(len(CH), len(Xs), Xs.shape[-1]).transpose(1, 0, 2).astype(np.float32)
    for e in EMOS:
        m = es == e
        if m.sum() >= 5: theta_tbl["filter"][(sid, e)] = region_theta(Xf[m])
    # ICA
    Xi, nic, ictypes = clean_session(Xs, do_asr=False)
    ic_removed[sid] = (nic, ictypes)
    print(f"   ICA removed {nic} components: {ictypes}")
    for e in EMOS:
        m = es == e
        if m.sum() >= 5: theta_tbl["ica"][(sid, e)] = region_theta(Xi[m])
    # ICA+ASR
    if ASR is not None:
        Xa, _, _ = clean_session(Xs, do_asr=True)
        for e in EMOS:
            m = es == e
            if m.sum() >= 5: theta_tbl["ica_asr"][(sid, e)] = region_theta(Xa[m])
